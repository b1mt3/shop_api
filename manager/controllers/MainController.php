...
class MainController extends DefaultController
{
	...
	public function parseHeader() {
		...
		if($authHeader){
			$app=  \frontend\modules\cashbox\models\App::find()->where(['auth_token'=>$authHeader])->one();
			if($app===null) {
				$this->responseStatus=  $this->returnResponse(421);
	...
	public function getErrorMessage($code, $data) {
		...
		$errMessModel=  \frontend\modules\manager\models\ErrorMessage::find()->where(['code'=>$code])->one();
